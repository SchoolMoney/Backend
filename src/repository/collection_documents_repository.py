...
async def create(
    session: AsyncSession, collection_doc: CreateCollectionDocumentDB
) -> CollectionDocument:
...
async def update(
    session: AsyncSession, collection_document: CollectionDocument
...
) -> Sequence[CollectionDocumentMetadata]:
    query = select(CollectionDocuments).filter(
        CollectionDocuments.collection_id == collection_id
    )
...
async def get_by_id(
    session: AsyncSession, collection_document_id: int
) -> Optional[CollectionDocument]:
    query = select(CollectionDocuments).filter(
        CollectionDocuments.document_id == collection_document_id
    )
...
async def delete(
    session: AsyncSession, collection_document_id: int, user: AuthorizedUser
) -> Optional[CollectionDocument]:
    try:
        ...
        if (
            not await check_if_user_can_view_collection(
                session, document.collection_id, user.user_id
            )
