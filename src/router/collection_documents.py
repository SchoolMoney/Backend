...
from src.Model.CollectionDocument import (
    CollectionDocument,
    CreateCollectionDocument,
    CreateCollectionDocumentDB,
...
@collection_documents_router.post(
    path="/",
...
async def create_collection_document(
    ...
    if (
        not await check_if_user_can_view_collection(
            sql_session, collection_document.collection_id, user.user_id
        )
    ...
    try:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid file_data"
            )
        ...
        result = await collection_documents_repository.create(
            sql_session, document_data
        )
...
@collection_documents_router.put(
    path="/", status_code=status.HTTP_200_OK, response_model=CollectionDocumentMetadata
)
async def update_collection_document(
    ...
    try:
        document = await collection_documents_repository.update(
            sql_session, collection_document
        )
...
@collection_documents_router.delete(
    path="/{document_id}",
...
async def delete_collection_document(
    ...
    try:
        deleted_document = await collection_documents_repository.delete(
            sql_session, document_id, user
        )
...
@collection_documents_router.get(
    path="/{document_id}",
    status_code=status.HTTP_200_OK,
    response_model=CollectionDocumentMetadata,
)
async def get_document_by_id(
    ...
    try:
        document = await collection_documents_repository.get_by_id(
            sql_session, document_id
        )
        ...
        if (
            not await check_if_user_can_view_collection(
                sql_session, document.collection_id, user.user_id
            )
            and user.user_privilege != ADMIN_USER
        ):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN)
...
async def get_collection_documents(
    ...
    try:
        documents = await collection_documents_repository.get(
            sql_session, collection_id
        )
...
@collection_documents_router.get(
    path="/{document_id}/content", status_code=status.HTTP_200_OK
)
async def get_document_content(
    ...
    try:
        ...
        encoded_filename = urllib.parse.quote(
            f"{document.document_name}.{document.file_type}"
        )
