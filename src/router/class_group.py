...
from src.SQL.Tables.OrganizationUnit import generate_random_code
...
@class_group_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=List[ClassGroup]
)
...
@class_group_router.get(
    "/{class_group_id}", status_code=status.HTTP_200_OK, response_model=ClassGroup
)
...
@class_group_router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=ClassGroup
)
...
async def update_class_group(
    ...
    try:
        await class_group_service.update(
            sql_session, class_group_id, request, user.user_id
        )
    except HTTPException:
        raise
    except Exception as e:
...
@class_group_router.put(
    "/{class_group_id}/cashier", status_code=status.HTTP_204_NO_CONTENT
)
async def change_class_group_cashier(
    ...
    try:
        await class_group_service.change_cashier(
            sql_session, class_group_id, request, user.user_id
        )
    except HTTPException:
        raise
...
@class_group_router.put(
    "/{class_group_id}/rotate_access_code", status_code=status.HTTP_204_NO_CONTENT
)
async def rotate_access_code(
    ...
    class_cashier = (
        await sql_session.exec(
            SQL.select(SQL.Tables.ParentGroupRole).where(
                SQL.Tables.ParentGroupRole.class_group_id == class_group_id,
                SQL.Tables.ParentGroupRole.role == ParentRole.CASHIER,
            )
        )
    ).first()
    ...
    if class_cashier.parent_id != parent_profile.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="User not allowed to rotate access code",
        )

    class_group = (
        await sql_session.exec(
            SQL.select(SQL.Tables.ClassGroup).where(
                SQL.Tables.ClassGroup.id == class_group_id
            )
        )
    ).first()

    class_group.access_code = generate_random_code()

    try:
        sql_session.add(class_group)
        await sql_session.commit()
    except Exception as e:
        logger.logger.error(e)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to rotate access code",
        )
...
async def get_class_view(
    ...
    class_view_data = await class_view_operations.collect_class_view_data(
        class_group_id, int(collection_status), user.user_id
    )

    if not class_view_data["class"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Class view not found"
        )
